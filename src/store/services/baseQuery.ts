...
// Exported so callers that need the raw backend origin outside RTK Query's baseQuery don't
// duplicate this fallback.
//
// Trailing slash trimmed here rather than at each call site: EXPO_PUBLIC_API_URL ends in one,
// so a hand-built `${API_URL}/...` produced a doubled slash that the backend answers with a
// 308 — which broke the old PayU redirect, since PayU POSTed to that URL without following
// redirects. RTK Query normalises its own joins either way.
...
export function createFallbackBaseQuery(withAuth = false): BaseQueryFn<string | FetchArgs, unknown, FetchBaseQueryError> {
  return fetchBaseQuery({
    ...
    // case is payment-adjacent: create-order hanging leaves the Pay button
